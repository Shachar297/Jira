...
async function getProject(projectKey) {
    fetch(`${configTest.testEnv.baseUrl}/project/${projectKey}`, {
        method: "GET",
        headers: {
...
async function login() {
    const parameters = {
        "username": configTest.testEnv.username,
        "password": configTest.testEnv.password
    }
    let loginInfo;
    let a = await fetch(`${configTest.testEnv.baseUrl}/auth/1/session`, {

        method: "POST",
        headers: {
            "User-Agent": configTest.testEnv.userAgent,
            "Content-Type": "application/Json"
        },
...
async function searchJql(parameters) {
    ...
    await fetch(configTest.testEnv.baseUrl + "search", {
        method: "POST",
        headers: {
            "User-Agent": configTest.testEnv.userAgent,
...
async function getIssueById(issueId, start, i) {
    ...
    let issue;
    fetch(configTest.testEnv.baseUrl + "issue/" + issueId, {
        method: "GET",
        headers: {
            "User-Agent": configTest.testEnv.userAgent,
            "Authorization": configTest.testEnv.token,
...
function setOptions(issueObject) {
    ...
    if (configTest.testEnv.baseUrl.indexOf('https') != -1) {
